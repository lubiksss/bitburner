...
import {getAvailableServers, getAvailableServerThreads, getHackableServers, scanAll} from "/src/utils/scan";
...
export async function main(ns) {
  ...
  while (true) {
    ...
    for (const targetServer of hackableServers) {
      ...
      //there is 01 max money, to filter it
      if (maxMoney < 1) {
        continue
      }
      ...
      //TODO
      // will they be stable, if grow level goes up?
      ns.tprint(hwgw)
      ns.tprint(neededThreads)
      ns.tprint(availableServerThreads)
      ns.exit()
